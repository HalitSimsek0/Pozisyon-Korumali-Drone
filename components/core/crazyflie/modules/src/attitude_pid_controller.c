...
void attitudeControllerInit(const float updateDt)
{
  ...
  pidInitFilters(&pidRoll, ATTITUDE_RATE, 30.0f, 50.0f, 100.0f);
  pidInitFilters(&pidPitch, ATTITUDE_RATE, 30.0f, 50.0f, 100.0f);
  pidInitFilters(&pidYaw, ATTITUDE_RATE, 30.0f, 50.0f, 100.0f);
  pidInitFilters(&pidRollRate, ATTITUDE_RATE, ATTITUDE_RATE_LPF_CUTOFF_FREQ, 0.0f, 0.0f);
  pidInitFilters(&pidPitchRate, ATTITUDE_RATE, ATTITUDE_RATE_LPF_CUTOFF_FREQ, 0.0f, 0.0f);
  pidInitFilters(&pidYawRate, ATTITUDE_RATE, ATTITUDE_RATE_LPF_CUTOFF_FREQ, 0.0f, 0.0f);
  
  // Load PID configuration from NVS (if available)
  pid_config_t pidConfig;
  pid_config_t defaults;
  pidNvsGetDefaults(&defaults);
  if (pidNvsLoad(&pidConfig, &defaults) == ESP_OK) {
...
LOG_ADD(LOG_FLOAT, roll_outF, &pidRoll.outF)
...
PARAM_ADD(PARAM_FLOAT, roll_kf, &pidRoll.kf)
